...
class ClusterFileTester(unittest.TestCase):
    """
    Tests the cluster file object.
    """
    ...
    def test_init(self):
        ...
        Make sure init can occur.
        """
        test_file = os.path.join("test_files", "cluster.txt")
        PortalFiles.ClusterFile(test_file)
        self.assertTrue(True, "Cluster file can init.")

    def test_check_header_correct(self):
        """
        Check the header is called correct when it is.
        """
        test_file_name = os.path.join("test_files", "cluster.txt")
        test_file = PortalFiles.ClusterFile(test_file_name)
        if test_file.file_has_error:
            self.assertTrue(False, "Did not start test with a no error state.")
        test_file.check_header()
        self.assertTrue(not test_file.file_has_error,
    ...
    def test_deidentify_cells(self):
        ...
        test_file_name = os.path.join("test_files", "cluster.txt")
        deid_file = os.path.join("test_files",
                                 "cluster"+PortalFiles.c_DEID_POSTFIX+".txt")
        map_file = os.path.join("test_files",
                                "cluster"+PortalFiles.c_MAP_POSTFIX+".txt")
        correct_file = os.path.join("test_files",
                                    "cluster_deidentifed_correct.txt")
        correct_map = os.path.join("test_files",
                                   "cluster_mapping_correct.txt")
        # Remove the test files if the exist
        if(os.path.exists(deid_file)):
            os.remove(deid_file)
        if(os.path.exists(map_file)):
    ...
    def test_deidentify_cells_precreated_names(self):
        ...
        map_file = os.path.join("test_files",
                                "cluster"+PortalFiles.c_MAP_POSTFIX+".txt")
        correct_file = os.path.join("test_files",
                                    "cluster_deidentifed_precreated_correct.txt")
        correct_map = os.path.join("test_files",
                                   "cluster_mapping_precreated_correct.txt")
        # Remove the test files if the exist
        if(os.path.exists(deid_file)):
            os.remove(deid_file)
        if(os.path.exists(map_file)):
